 ...
 */
export class CuddleCoind extends HTTPClient implements CuddleCoindTypes.ICuddleCoind {
    /**
    ...
    public async transactionsStatus (
        transactions: string[]
    ): Promise<CuddleCoindTypes.ITransactionsStatus> {
    ...
    public async randomIndexes (
        amounts: number[],
        count = 3
    ): Promise<CuddleCoindTypes.IRandomOutput[]> {
    ...
    public async indexes (
        startHeight: number,
        endHeight: number
    ): Promise<CuddleCoindTypes.ITransactionIndexes[]> {
        return this.get('indexes/' + startHeight + '/' + endHeight)
            .then((result: CuddleCoindTypes.ITransactionIndexes[]) =>
    ...
    public async sync (
        ...
        count = 100): Promise<CuddleCoindTypes.ISync> {
        ...
        response.blocks = response.blocks.map((block: CuddleCoindTypes.ISyncBlock) => {
    ...
    public async rawSync (
        ...
        timestamp = 0,
        skipCoinbaseTransactions = false,
        count = 100): Promise<CuddleCoindTypes.IRawSync> {
