...
import { ED25519, CuddleCoinCrypto } from './Types';
...
export class Address {
    ...
    public static async fromAddress (address: string, prefix?: AddressPrefix | number): Promise<Address> {
        ...
        const checksum = (new Reader(
            await CuddleCoinCrypto.cn_fast_hash(decodedPrefix + paymentId + publicSpend + publicView)
    ...
    public static async fromSeed (seed: string, language?: string, prefix?: AddressPrefix | number): Promise<Address> {
        ...
        if (!Common.isHex64(seed)) {
            seed = await CuddleCoinCrypto.cn_fast_hash(seed);
        }
    ...
    public static async generateSubwallet (
        ...
        if (!await CuddleCoinCrypto.checkScalar(privateSpendKey)) {
        ...
        const spend = await CuddleCoinCrypto.generateDeterministicSubwalletKeys(privateSpendKey, subwalletIndex);
    ...
    public async toString (): Promise<string> {
        ...
        const checksum = (await CuddleCoinCrypto.cn_fast_hash(writer.blob))
            .slice(0, 8);
