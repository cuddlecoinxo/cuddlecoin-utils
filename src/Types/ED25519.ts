...
import { CuddleCoinCrypto } from '../Types';
...
export namespace ED25519 {
    ...
    export class KeyPair {
        ...
        public static async from (
            ...
            if (publicKey && await CuddleCoinCrypto.checkKey(publicKey)) {
                pair.m_publicKey = publicKey;
            }

            if (privateKey && await CuddleCoinCrypto.checkScalar(privateKey)) {
            ...
            if (pair.m_privateKey && !pair.m_publicKey) {
                ...
                if (iterations && iterations === 1) {
                    const temp = await CuddleCoinCrypto.cn_fast_hash(
                        pair.m_privateKey);

                    await pair.setPrivateKey(temp);
                }

                pair.m_publicKey = await CuddleCoinCrypto.secretKeyToPublicKey(pair.m_privateKey);
            }
        ...
        public async setPrivateKey (key: string): Promise<void> {
            try {
                this.m_privateKey = (await CuddleCoinCrypto.checkScalar(key))
                    ? key
                    : await CuddleCoinCrypto.scReduce32(key);
            } catch (e) {
        ...
        public async setPublicKey (key: string): Promise<void> {
            ...
            try {
                isPubKey = await CuddleCoinCrypto.checkKey(key);
            } catch (e) {
        ...
        public async isPaired (): Promise<boolean> {
            ...
            return (await CuddleCoinCrypto.secretKeyToPublicKey(this.privateKey) === this.publicKey);
        }
    }
...
async function simpleKdf (value: string, iterations: number): Promise<string> {
    ...
    for (let i = 0; i < iterations; i++) {
        hex = await CuddleCoinCrypto.cn_fast_hash(hex);
    }
    return hex;
}
