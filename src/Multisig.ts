...
import { Interfaces, MultisigInterfaces, TransactionInputs, CuddleCoinCrypto } from './Types';
...
export class Multisig {
    ...
    public async view (): Promise<string> {
        ...
        return CuddleCoinCrypto.calculateSharedPrivateKey(this.m_view_keys);
    }
    ...
    public async spend (): Promise<string> {
        ...
        return CuddleCoinCrypto.calculateSharedPublicKey(keys);
    }
    ...
    private async calculated_multisig_keys (): Promise<KeyPair[]> {
        if (this.m_threshold !== this.m_participants) {
            ...
            for (const multisig_key of this.m_wallet_multisig_keys) {
                const keys =
                    await CuddleCoinCrypto.calculateMultisigPrivateKeys(
    ...
    public static async fromMultisigKeys (
        ...
        for (const key of multisig_private_keys) {
            if (!await CuddleCoinCrypto.checkScalar(key)) {
        ...
        if (!await CuddleCoinCrypto.checkScalar(sharedPrivateViewKey)) {
    ...
    public static async restoreKeyImage (
        ...
        return CuddleCoinCrypto.restoreKeyImage(publicEphemeral, derivation, outputIndex, partialKeyImages);
    }
    ...
    public async addParticipant (
        ...
        if (privateViewKey && !await CuddleCoinCrypto.checkScalar(privateViewKey)) {
            throw new Error('Private view key is not a valid private key');
        }
        ...
        for (const key of publicSpendKeys) {
            if (!await CuddleCoinCrypto.checkKey(key)) {
    ...
    public async generatePartialKeyImages (
        ...
        for (const multisigKey of this.m_multisig_keys) {
            promises.push(CuddleCoinCrypto.generateKeyImage(publicEphemeral, multisigKey.privateKey));
        }
...
async function generatePartialSigningKey (
    ...
    const key = await CuddleCoinCrypto.generatePartialSigningKey(preparedSignature, privateSpendKey);
...
async function restoreRingSignatures (
    ...
    const sigs = await CuddleCoinCrypto.restoreRingSignatures(
        derivation,
        outputIndex,
        keys,
        realOutputIndex,
