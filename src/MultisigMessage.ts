...
import { ED25519, Interfaces as TransactionInterfaces, MultisigInterfaces, CuddleCoinCrypto } from './Types';
...
export class MultisigMessage {
    ...
    public static async decode (destination: Address, base58: string): Promise<MultisigMessage> {
        ...
        const hash = await CuddleCoinCrypto.cn_fast_hash(rawData);
        ...
        if (!await CuddleCoinCrypto.checkSignature(hash, source.spend.publicKey, signature)) {
    ...
    public async encode (): Promise<string> {
        ...
        const hash = await CuddleCoinCrypto.cn_fast_hash(writer.blob);

        const sig = await CuddleCoinCrypto.generateSignature(
...
async function encrypt (
    ...
    const aesKey = Buffer.from(
        await CuddleCoinCrypto.generateKeyDerivation(
...
async function calculateSpendKeySignatures (
    ...
    for (const keys of spendKeys) {
        ...
        const sig = await CuddleCoinCrypto.generateSignature(keys.publicKey, keys.publicKey, keys.privateKey);
...
async function verifySpendKeySignatures (spendKeys: MultisigInterfaces.PublicSpendKey[]): Promise<void> {
    for (const spendKey of spendKeys) {
        if (!await CuddleCoinCrypto.checkSignature(spendKey.key, spendKey.key, spendKey.signature)) {
