...
import { TransactionInputs, TransactionOutputs, CuddleCoinCrypto } from './Types';
...
export class Block {
    ...
    public async baseTransactionBranch (): Promise<string[]> {
        ...
        return CuddleCoinCrypto.tree_branch(transactions);
    }
    ...
    public async transactionTreeHash (): Promise<{ hash: string, count: number }> {
        ...
        const treeHash = await CuddleCoinCrypto.tree_hash(transactions);
    ...
    public static async from (data: Buffer | string, config?: ICoinConfig): Promise<Block> {
        ...
        if (block.m_majorVersion >= block.m_config.activateParentBlockVersion) {
            ...
            const baseTransactionBranchDepth = await CuddleCoinCrypto.tree_depth(block.m_parentBlock.transactionCount);
    ...
    public async toHashingBuffer (headerOnly = false): Promise<Buffer> {
        ...
        if (this.m_majorVersion >= this.m_config.activateParentBlockVersion) {
            ...
            const treeHash = await CuddleCoinCrypto.tree_hash_from_branch(
...
async function getBlockHash (data: Buffer): Promise<string> {
    ...
    return CuddleCoinCrypto.cn_fast_hash(writer.blob);
}
...
async function getBlockPoWHash (data: Buffer, majorVersion: number): Promise<string> {
    ...
    switch (majorVersion) {
        ...
        case 2:
        case 3:
            return CuddleCoinCrypto.cn_slow_hash_v0(blob);
        case 4:
            return CuddleCoinCrypto.cn_lite_slow_hash_v1(blob);
        case 5:
            return CuddleCoinCrypto.cn_turtle_lite_slow_hash_v2(blob);
        case 6:
            return CuddleCoinCrypto.chukwa_slow_hash_v1(blob);
        case 7:
            return CuddleCoinCrypto.chukwa_slow_hash_v2(blob);
        default:
