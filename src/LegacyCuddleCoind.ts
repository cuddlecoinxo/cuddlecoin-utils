// Copyright (c) 2018-2020, Brandon Lehmann, The CuddleCoin Developers
//
// Please see the included LICENSE file for more information.

import { HTTPClient } from './Helpers/HTTPClient';
import { LegacyCuddleCoindTypes } from './Types/LegacyCuddleCoind';
...
/**
 * A class interface that allows for easy interaction with Legacy CuddleCoind
 * THIS OBJECT IS DEPRECATED AND SUBJECT TO REMOVAL WITH LITTLE NOTICE
 */
export class LegacyCuddleCoind extends HTTPClient implements CuddleCoindTypes.ICuddleCoind {
    /**
     * Retrieves the node fee information
     */
    public async fee (): Promise<CuddleCoindTypes.IFee> {
        const response: LegacyCuddleCoindTypes.IFeeResponse = await this.get('fee');
     ...
     */
    public async height (): Promise<CuddleCoindTypes.IHeight> {
        const response: LegacyCuddleCoindTypes.IHeightResponse = await this.get('height');
     ...
     */
    public async info (): Promise<CuddleCoindTypes.IInfo> {
        const response: LegacyCuddleCoindTypes.IInfoResponse = await this.get('info');

        const parse = (elem: string): CuddleCoindTypes.IVersion => {
     ...
     */
    public async peers (): Promise<CuddleCoindTypes.IPeers> {
        const response: LegacyCuddleCoindTypes.IPeersResponse = await this.get('peers');
     ...
     */
    public async block (block: string | number): Promise<CuddleCoindTypes.IBlock> {
        ...
        const response: LegacyCuddleCoindTypes.IBlockSummary = await this.rpcPost('f_block_json', { hash: block })
            .then(response => response.block);

        const result: CuddleCoindTypes.IBlock = {
     ...
     */
    public async lastBlock (): Promise<CuddleCoindTypes.IBlock> {
     ...
     */
    public async blockHeaders (height: number): Promise<CuddleCoindTypes.IBlock[]> {
        const response = await this._blockShortHeaders(height);

        const promises: Promise<CuddleCoindTypes.IBlock>[] = response.map(block => this.block(block.hash));
     ...
     * @param block the block height or hash
     */
    public async rawBlock (block: string | number): Promise<CuddleCoindTypes.IRawBlock> {
        let header: LegacyCuddleCoindTypes.IBlockHeader;
    ...
    public async blockTemplate (
        address: string,
        reserveSize = 8
    ): Promise<CuddleCoindTypes.IBlockTemplate> {
        const response: LegacyCuddleCoindTypes.IBlockTemplate = await this.rpcPost(
     ...
     */
    public async transaction (hash: string): Promise<CuddleCoindTypes.ITransaction> {
        const response = await this._transaction(hash);

        const toOutput = (output: LegacyCuddleCoindTypes.IOutputKey): CuddleCoindTypes.ITransactionOutputKey => {
            return {
        ...
        const toCoinbaseInput =
            (input: LegacyCuddleCoindTypes.IInputCoinbase): CuddleCoindTypes.ITransactionInputCoinbase => {
                return {
        ...
        const toKeyInput =
            (input: LegacyCuddleCoindTypes.IInputKey): CuddleCoindTypes.ITransactionInputKey => {
                return {
        ...
        for (const input of response.tx.vin) {
            switch (input.type) {
                case 'ff':
                    coinbase = true;
                    inputs.push(toCoinbaseInput(input as LegacyCuddleCoindTypes.IInputCoinbase));
                    break;
                case '02':
                    inputs.push(toKeyInput(input as LegacyCuddleCoindTypes.IInputKey));
                    break;
            }
        }
        ...
        return {
            ...
            prefix: {
                ...
                inputs: (coinbase)
                    ? inputs as CuddleCoindTypes.ITransactionInputCoinbase[]
                    : inputs as CuddleCoindTypes.ITransactionInputKey[],
     ...
     * in the memory pool
     */
    public async transactionPool (): Promise<CuddleCoindTypes.TransactionSummary[]> {
    ...
    public async transactionPoolChanges (
        ...
        transactions: string[] = []
    ): Promise<CuddleCoindTypes.ITransactionPoolDelta> {
        const response = await this._poolChanges(lastKnownBlock, transactions);

        const result: CuddleCoindTypes.ITransactionPoolDelta = {
            added: [],
        ...
        for (const added of response.addedTxs) {
            ...
            const addCoinbase = (input: LegacyCuddleCoindTypes.IInputCoinbase) =>
                tx.inputs.push(new CoinbaseInput(input.value.height));

            const addKeyInput = (input: LegacyCuddleCoindTypes.IInputKey) =>
            ...
            for (const input of added.prefix.vin) {
                switch (input.type) {
                    case 'ff':
                        addCoinbase(input as LegacyCuddleCoindTypes.IInputCoinbase);
                        break;
                    case '02':
                        addKeyInput(input as LegacyCuddleCoindTypes.IInputKey);
                        break;
                }
            }
     ...
     */
    public async transactionsStatus (transactions: string[]): Promise<CuddleCoindTypes.ITransactionsStatus> {
     ...
     */
    public async randomIndexes (amounts: number[], count: number): Promise<CuddleCoindTypes.IRandomOutput[]> {
        const response = await this._randomOutputs(amounts, count);

        const results: CuddleCoindTypes.IRandomOutput[] = [];
     ...
     */
    public async indexes (startHeight: number, endHeight: number): Promise<CuddleCoindTypes.ITransactionIndexes[]> {
        ...
        const results: CuddleCoindTypes.ITransactionIndexes[] = [];
    ...
    public async rawSync (
        ...
        skipCoinbaseTransactions = false,
        count = 100
    ): Promise<CuddleCoindTypes.IRawSync> {
        const response = await this._rawBlocks(height, timestamp, checkpoints, skipCoinbaseTransactions, count);

        const toRawBlock = (rawBlock: LegacyCuddleCoindTypes.IRawBlock): CuddleCoindTypes.IRawBlock => {
            return {
                blob: rawBlock.block,
        ...
        const result: CuddleCoindTypes.IRawSync = {
     ...
     */
    private async _lastBlockHeader (): Promise<LegacyCuddleCoindTypes.IBlockHeader> {
     ...
     */
    private async _blockHeaderByHash (hash: string): Promise<LegacyCuddleCoindTypes.IBlockHeader> {
     ...
     */
    private async _blockHeaderByHeight (height: number): Promise<LegacyCuddleCoindTypes.IBlockHeader> {
     ...
     */
    private async _blockShortHeaders (height: number): Promise<LegacyCuddleCoindTypes.IBlockShortHeader[]> {
    ...
    private async _globalIndexesForRange (
        startHeight: number,
        endHeight: number
    ): Promise<LegacyCuddleCoindTypes.IGlobalIndexesResponse[]> {
    ...
    private async _poolChanges (
    ...
    ): Promise<LegacyCuddleCoindTypes.IPoolChanges> {
        const body: any = {
        ...
        const tmp: LegacyCuddleCoindTypes.IBlockLiteTransaction[] = [];
    ...
    private async _randomOutputs (
        amounts: number[],
        mixin = 1
    ): Promise<LegacyCuddleCoindTypes.IRandomOutputsResponse> {
    ...
    private async _rawBlocks (
        ...
        skipCoinbaseTransactions = false,
        blockCount = 100
    ): Promise<LegacyCuddleCoindTypes.IRawBlocksResponse> {
    ...
    private async _sendRawTransaction (
        transaction: string
    ): Promise<LegacyCuddleCoindTypes.ISendRawTransactionResponse> {
     ...
     */
    private async _transaction (hash: string): Promise<LegacyCuddleCoindTypes.ITransactionResponse> {
     ...
     */
    private async _transactionPool (): Promise<LegacyCuddleCoindTypes.ITransactionSummary[]> {
    ...
    ): Promise<LegacyCuddleCoindTypes.ITransactionStatusResponse> {
    ...
    private async _walletSyncData (
        ...
        skipCoinbaseTransactions = false,
        blockCount = 100
    ): Promise<LegacyCuddleCoindTypes.IWalletSyncData> {
