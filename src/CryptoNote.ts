...
export class CryptoNote extends EventEmitter implements ICryptoNote {
    ...
    constructor (config?: ICoinConfig, cryptoConfig?: ICryptoConfig) {
        ...
        if (cryptoConfig) {
            CuddleCoinCrypto.userCryptoFunctions = cryptoConfig;
        }
    }
    ...
    public get cryptoConfig (): ICryptoConfig {
        return CuddleCoinCrypto.userCryptoFunctions;
    }

    public set cryptoConfig (config: ICryptoConfig) {
        CuddleCoinCrypto.userCryptoFunctions = config;
    }
    ...
    public async generateKeyDerivation (
        ...
        return CuddleCoinCrypto.generateKeyDerivation(
    ...
    public async generateKeyImage (
        ...
        const derivation = await CuddleCoinCrypto.generateKeyDerivation(transactionPublicKey, privateViewKey);
    ...
    public async generateKeyImagePrimitive (
        ...
        const publicEphemeral = await CuddleCoinCrypto.derivePublicKey(derivation, outputIndex, publicSpendKey);

        const privateEphemeral = await CuddleCoinCrypto.deriveSecretKey(derivation, outputIndex, privateSpendKey);

        const keyImage = await CuddleCoinCrypto.generateKeyImage(publicEphemeral, privateEphemeral);
    ...
    public async privateKeyToPublicKey (privateKey: string): Promise<string> {
        return CuddleCoinCrypto.secretKeyToPublicKey(privateKey);
    }
    ...
    public async isOurTransactionOutput (
        ...
        const derivedKey = await CuddleCoinCrypto.generateKeyDerivation(transactionPublicKey, privateViewKey);

        const publicEphemeral = await CuddleCoinCrypto.derivePublicKey(derivedKey, output.index, publicSpendKey);
        ...
        if (publicEphemeral === output.key) {
            ...
            if (privateSpendKey) {
                ...
                const privateEphemeral = (generatePartial)
                    ? privateSpendKey
                    : await CuddleCoinCrypto.deriveSecretKey(
                        derivedKey, output.index, privateSpendKey);

                const derivedPublicEphemeral = await CuddleCoinCrypto.secretKeyToPublicKey(privateEphemeral);
    ...
    public async signMessage (message: any, privateKey: string): Promise<string> {
        ...
        const publicKey = await CuddleCoinCrypto.secretKeyToPublicKey(privateKey);

        const hex = Buffer.from(message);

        const hash = await CuddleCoinCrypto.cn_fast_hash(hex.toString('hex'));

        return CuddleCoinCrypto.generateSignature(hash, publicKey, privateKey);
    }
    ...
    public async verifyMessageSignature (message: any, publicKey: string, signature: string): Promise<boolean> {
        ...
        return CuddleCoinCrypto.checkSignature(hash, publicKey, signature);
    }
...
async function checkRingSignatures (
    ...
    return CuddleCoinCrypto.checkRingSignatures(hash, keyImage, publicKeys, signatures);
}
...
async function generateRingSignatures (
    ...
    const signatures = await CuddleCoinCrypto.generateRingSignatures(
        hash,
        keyImage,
        publicKeys,
        privateKey,
...
async function prepareRingSignatures (
    ...
    const prepped = await CuddleCoinCrypto.prepareRingSignatures(
...
async function completeRingSignatures (
    ...
    const signatures = await CuddleCoinCrypto.completeRingSignatures(privateEphemeral, realOutputIndex, key, sigs);
...
async function prepareTransactionOutputs (outputs: Interfaces.GeneratedOutput[]): Promise<Interfaces.PreparedOutputs> {
    async function prepareOutput (
        ...
        const outDerivation = await CuddleCoinCrypto.generateKeyDerivation(destination.view.publicKey, privateKey);

        const outPublicEphemeral = await CuddleCoinCrypto.derivePublicKey(
            outDerivation,
            index,
