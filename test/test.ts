...
if (process.env.FORCE_JS) {
    if (CuddleCoinCrypto.forceJSCrypto()) {
...
describe('Cryptography', async function () {
    ...
    it('Generate Random Keys', async () => {
        const keys = await CuddleCoinCrypto.generateKeys();
    ...
    it('Check Key - Public Key', async () => {
        ...
        assert(await CuddleCoinCrypto.checkKey(key));
    });
    ...
    it('Tree Hash', async () => {
        ...
        const treeHash = await CuddleCoinCrypto.tree_hash([
    ...
    it('Tree Branch', async () => {
        ...
        const treeBranch = await CuddleCoinCrypto.tree_branch([
            'b542df5b6e7f5f05275c98e7345884e2ac726aeeb07e03e44e0389eb86cd05f0',
            '1b606a3f4a07d6489a1bcd07697bd16696b61c8ae982f61a90160f4e52828a7f',
            'c9fae8425d8688dc236bcdbc42fdb42d376c6ec190501aa84b04a4b4cf1ee122',
            '871fcd6823f6a879bb3f33951c8e8e891d4043880b02dfa1bb3be498b50e7578'
...
describe('SubWallets', async function () {
    ...
    it('Subwallet #2 not found from Subwallet #1', async () => {
        const key = await CuddleCoinCrypto.cn_fast_hash(subWallets[1].spend.privateKey);
        assert(key !== subWallets[2].spend.privateKey);
        assert(await CuddleCoinCrypto.scReduce32(key) !== subWallets[2].spend.privateKey);
    });

    it('Subwallet #64 not found from Subwallet #1', async () => {
        let key = subWallets[1].spend.privateKey;
        for (let i = 0; i < 63; i++) {
            key = await CuddleCoinCrypto.cn_fast_hash(key);
        }
        assert(key !== subWallets[3].spend.privateKey);
        assert(await CuddleCoinCrypto.scReduce32(key) !== subWallets[3].spend.privateKey);
    });
    ...
    it('Subwallet #65 not found from Subwallet #1', async () => {
        ...
        assert(await CuddleCoinCrypto.scReduce32(key) !== subWallets[4].spend.privateKey);
    });
...
describe('Transactions', async function () {
    ...
    describe('Output Discovery', async () => {
        ...
        before('Generate Derivation', async () => {
            derivation = await CuddleCoinCrypto.generateKeyDerivation(txPublicKey, walletPrivateViewKey);
        });

        it('underive public spend key (no match)', async () => {
            const publicSpendKey1 = await CuddleCoinCrypto.underivePublicKey(
        ...
        it('underive public spend key (match)', async () => {
            const publicSpendKey2 = await CuddleCoinCrypto.underivePublicKey(
...
describe('Test Ledger Integration', async function () {
    let skipLedgerTests = CuddleCoinCrypto.type !== CryptoType.NODEADDON;
    ...
    it('Generate Key Image', async function () {
        ...
        const derivation = await CuddleCoinCrypto.generateKeyDerivation(
            ledger.address.view.publicKey, keys.private_key);

        const public_ephemeral = await CuddleCoinCrypto.derivePublicKey(
            derivation, 0, ledger.address.spend.publicKey);

        const private_ephemeral = await CuddleCoinCrypto.deriveSecretKey(
            derivation, 0, spend_key.privateKey);

        const key_image = await CuddleCoinCrypto.generateKeyImage(public_ephemeral, private_ephemeral);
    ...
    it('Sign a message', async function () {
        ...
        const message = { ledger: 'CuddleCoin Rocks!' };
    ...
    it('Create a Transaction', async function () {
        ...
        for (let i = 0; i < 3; i++) {
            const random = await CuddleCoinCrypto.generateKeys();
...
describe('CuddleCoind < 1.0.0', function () {
    ...
    const server = new LegacyCuddleCoind('149.28.168.93');
...
describe('WalletAPI', async function () {
    ...
    describe('New Wallet', () => {
        ...
        it('setNode()', async () => {
            await server.setNode('localhost', 12898);
        });
